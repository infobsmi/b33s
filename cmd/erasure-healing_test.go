...
func TestHealCorrectQuorum(t *testing.T) {
	...
	for _, set := range z.serverPools {
		...
		fileInfos, errs = readAllFileInfo(ctx, erasureDisks, b33sMetaBucket, cfgFile, "", false)
		nfi, err = getLatestFileInfo(ctx, fileInfos, er.defaultParityCount, errs)
		if errors.Is(err, errFileNotFound) {
			continue
		}
		...
		for i := 0; i < nfi.Erasure.ParityBlocks; i++ {
			erasureDisks[i].Delete(context.Background(), b33sMetaBucket, pathJoin(cfgFile, xlStorageFormatFile), DeleteOptions{
				Recursive: false,
				Force:     false,
			})
		}
		...
		// Try healing now, it should heal the content properly.
		_, err = objLayer.HealObject(ctx, b33sMetaBucket, cfgFile, "", hopts)
		if err != nil {
			t.Fatal(err)
		}
