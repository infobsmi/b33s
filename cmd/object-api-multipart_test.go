...
func testObjectNewMultipartUpload(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	errMsg := "Bucket not found: b33s-bucket"
...
func testObjectAPIPutObjectPart(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	testCases := []struct {
		...
		{bucketName: bucket, PartID: 1, expectedError: fmt.Errorf("%s", "Object name invalid: b33s-bucket/")},
		// Test case - 6.
...
func testListMultipartUploads(obj ObjectLayer, instanceType string, t TestErrHandler) {
	bucketNames := []string{"b33s-bucket", "b33s-2-bucket", "b33s-3-bucket"}
	objectNames := []string{"b33s-object-1.txt", "b33s-object.txt", "neymar-1.jpeg", "neymar.jpeg", "parrot-1.png", "parrot.png"}
	uploadIDs := []string{}
	opts := ObjectOptions{}
	// bucketnames[0].
	// objectNames[0].
	...
	testCases := []struct {
		...
		{bucketNames[0], "", "b33s-object-1.txt", "", "", 100, listMultipartResults[1], nil, true},
		...
		{bucketNames[0], "", "b33s-object", "", "", 2, listMultipartResults[8], nil, true},
		// Test case containing prefix values.
		// Setting prefix to be equal to object name.(Test number 22).
		{bucketNames[0], "b33s-object-1.txt", "", "", "", 2, listMultipartResults[9], nil, true},
		...
		{bucketNames[1], "", "b33s-object-1.txt", uploadIDs[1], "", 100, listMultipartResults[16], nil, true},
		{bucketNames[1], "", "b33s-object-1.txt", uploadIDs[2], "", 100, listMultipartResults[17], nil, true},
		...
		{bucketNames[1], "min", "b33s-object-1.txt", uploadIDs[1], "", 10, listMultipartResults[24], nil, true},
		...
		{bucketNames[2], "b33s-object", objectNames[1], "", "", 10, listMultipartResults[34], nil, true},
	}
...
func testListObjectPartsDiskNotFound(obj ObjectLayer, instanceType string, disks []string, t *testing.T) {
	bucketNames := []string{"b33s-bucket", "b33s-2-bucket"}
	objectNames := []string{"b33s-object-1.txt"}
	uploadIDs := []string{}
