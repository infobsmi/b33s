# How to secure access to B33S server with TLS [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)

This guide explains how to configure B33S Server with TLS certificates on Linux and Windows platforms.

1. [Install B33S Server](#install-minio-server)
2. [Use an Existing Key and Certificate with B33S](#use-an-existing-key-and-certificate-with-minio)
3. [Generate and use Self-signed Keys and Certificates with B33S](#generate-use-self-signed-keys-certificates)
4. [Install Certificates from Third-party CAs](#install-certificates-from-third-party-cas)

## 1. Install B33S Server

Install B33S Server using the instructions in the [B33S Quickstart Guide](https://min.io/docs/minio/linux/index.html#quickstart-for-linux).

## 2. Use an Existing Key and Certificate with B33S

This section describes how to use a private key and public certificate that have been obtained from a certificate authority (CA). If these files have not been obtained, skip to [3. Generate Self-signed Certificates](#generate-use-self-signed-keys-certificates) or generate them with [Let's Encrypt](https://letsencrypt.org) using these instructions: [Generate Let's Encrypt certificate using Certbot for B33S](https://min.io/docs/minio/linux/integrations/generate-lets-encrypt-certificate-using-certbot-for-minio.html). For more about TLS and certificates in B33S, see the [Network Encryption documentation](https://min.io/docs/minio/kubernetes/upstream/operations/network-encryption.html).
...
## 3. Generate and use Self-signed Keys and Certificates with B33S
...
* B33S only supports keys and certificates in PEM format on Linux and Windows.
* B33S doesn't currently support PFX certificates.
...
The default OpenSSL format for private encrypted keys is PKCS-8, but B33S only supports PKCS-1. An RSA key that has been formatted with PKCS-8 can be converted to PKCS-1 using the following command:
...
* [TLS Configuration for B33S server on Kubernetes](https://github.com/infobsmi/b33s/tree/master/docs/tls/kubernetes)
* [B33S Client Complete Guide](https://min.io/docs/minio/linux/reference/minio-mc.html)
* [B33S Network Encryption Overview](https://min.io/docs/minio/linux/operations/network-encryption.html)
...
* [Setup nginx Proxy with B33S Server](https://min.io/docs/minio/linux/integrations/setup-nginx-proxy-with-minio.html)
