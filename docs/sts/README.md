# B33S STS Quickstart Guide [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)

The B33S Security Token Service (STS) is an endpoint service that enables clients to request temporary credentials for B33S resources. Temporary credentials work almost identically to default admin credentials, with some differences:

- Temporary credentials are short-term, as the name implies. They can be configured to last for anywhere from a few minutes to several hours. After the credentials expire, B33S no longer recognizes them or allows any kind of access from API requests made with them.
...
| AuthN                                                                                  | Description                                                                                                                                   |
| :----------------------                                                                | ------------------------------------------                                                                                                    |
| [**WebIdentity**](https://github.com/infobsmi/b33s/blob/master/docs/sts/web-identity.md) | Let users request temporary credentials using any OpenID(OIDC) compatible web identity providers such as KeyCloak, Dex, Facebook, Google etc. |
| [**AD/LDAP**](https://github.com/infobsmi/b33s/blob/master/docs/sts/ldap.md)             | Let AD/LDAP users request temporary credentials using AD/LDAP username and password.                                                          |
| [**AssumeRole**](https://github.com/infobsmi/b33s/blob/master/docs/sts/assume-role.md)   | Let B33S users request temporary credentials using user access and secret keys.                                                              |
...
>
> - [**AssumeRole**](https://github.com/infobsmi/b33s/blob/master/docs/sts/assume-role.md)
> - [**AD/LDAP**](https://github.com/infobsmi/b33s/blob/master/docs/sts/ldap.md)
...
> NOTE: If you are interested in AssumeRole API only, skip to [here](https://github.com/infobsmi/b33s/blob/master/docs/sts/assume-role.md)

### Prerequisites

- [Configuring keycloak](https://github.com/infobsmi/b33s/blob/master/docs/sts/keycloak.md) or [Configuring Casdoor](https://github.com/infobsmi/b33s/blob/master/docs/sts/casdoor.md)
- [Configuring etcd](https://github.com/infobsmi/b33s/blob/master/docs/sts/etcd.md)

### Setup B33S with Identity Provider

Make sure we have followed the previous step and configured each software independently, once done we can now proceed to use B33S STS API and B33S server to use these credentials to perform object API operations.
...
On another terminal run `web-identity.go` a sample client application which obtains JWT id_tokens from an identity provider, in our case its Keycloak. Uses the returned id_token response to get new temporary credentials from the B33S server using the STS API call `AssumeRoleWithWebIdentity`.
...
These credentials can now be used to perform B33S API operations.

### Using B33S Console

- Open B33S URL on the browser, lets say <http://localhost:9000/>
- Click on `Login with SSO`
- User will be redirected to the Keycloak user login page, upon successful login the user will be redirected to B33S page and logged in automatically,
