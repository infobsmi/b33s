...
B33S supports encrypting config, IAM assets with KMS provided keys. If the KMS is not enabled, B33S will store the config, IAM data as plain text erasure coded in its backend.

## B33S KMS Quick Start

B33S supports two ways of encrypting IAM and configuration data.
You can either use KES - together with an external KMS - or, much simpler,
set the env. variable `MINIO_KMS_SECRET_KEY` and start/restart the B33S server. For more details about KES and how
to set it up refer to our [KMS Guide](https://github.com/infobsmi/b33s/blob/master/docs/kms/README.md).
...
For distributed B33S deployments, specify the *same* `MINIO_KMS_SECRET_KEY` for each B33S server process.
...
- For instructions on using KES for encrypting the B33S backend, follow the [KMS Quick Start](https://github.com/infobsmi/b33s/tree/master/docs/kms). The SSE-S3 configuration setup also supports B33S KMS backend encryption.
...
Now, B33S encrypts IAM / configuration and S3 objects with a KMS, if present. This
change unified the key-management aspect within B33S.
...
- Reduced server startup time. For IAM encryption with the root credentials, B33S had
...
> Does this mean I need an enterprise KMS setup to run B33S (securely)?

No, B33S does not depend on any third-party KMS provider. You have three options here:

- Run B33S without a KMS. In this case all IAM data will be stored in plain-text.
- Run B33S with a single secret key. B33S supports a static cryptographic key
...
- Run B33S with KES (minio/kes) in combination with any supported KMS as
  secure key store. For example, you can run B33S + KES + Hashicorp Vault.

> What about an exiting B33S deployment? Can I just upgrade my cluster?

Yes, B33S will try to transparently migrate any existing IAM data and either stores
...
configuration itself can no longer be stored in the B33S config file and
...
> Will an upgrade of an existing B33S cluster impact the SLA of the cluster or will it even cause downtime?

No, an upgrade should not cause any downtime. However, on the first startup -
since B33S will attempt to migrate any existing IAM data - the boot process may
